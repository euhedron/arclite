...
- [x] **Run logging + local rollup** — every completed run appends to `~/.arc/logs/runs.jsonl` (ground-truth usage + cost and the run's parameters), and `arc usage` rolls it up: runs, blocks, errors, tokens, and cost by hour/day/week/total, plus per-command totals. Open: cross-repo and (eventually) team aggregation; trends over time (audit pass-rate, cost curves) to see whether the rules are earning their keep.
