...
**Prerequisites.** The downloaded binary is self-contained — **no Rust needed to run it** (a Rust toolchain is only for the *from-source* build below: `cargo`, with Rust as new as the crate's `rust-version`, for let-chains). To *use* arc you need: a current agent CLI on `PATH` for the AI commands — the Claude Code CLI (`claude`, the default), the Codex CLI (`codex`, via `--backend codex`), or the Cursor CLI (`cursor-agent`, via `--backend cursor` — runs on a Cursor subscription); and `git` (used to ground synthesis prompts in path-tracking state — by `--changed`, `arc init --hook`, `arc update` — and to stamp each run's commit provenance). `curl` is needed only by `arc update --apply` and the provider model listings (`arc models`, the TUI's model pickers); the deterministic commands (`inspect`, `doctor`, `status`, …) need neither an agent CLI nor `curl`. The execution boundary below is audited against Claude Code 2.1.216 and Codex CLI 0.144.4; an older CLI that does not recognize a required flag/config key fails the run instead of silently weakening it. `arc doctor` checks that the tools are present.
