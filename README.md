...
- [ ] **Promote findings into the repo** — collect proven results under the repo's `.arc/findings/` (and/or `~/.arc/`) as a curated, per-scope ledger of accepted, still-open findings. **`arc promote <run-id>` landed (2026-06-23):** the *system* writes a logged run's structured findings into `.arc/findings/open/` (atomic, collision-free names — agents invoke arc, they don't hand-author the ledger). Still open: a **verify lever** that re-checks promoted findings still reproduce (acting on a finding stales it — fixed ones leave the ledger, via the system, not by hand); a **run parameter that feeds the ledger into context** so a run hunts *beyond* the already-known findings; and cross-scope curation (repo vs. `~/.arc`). Floated 2026-06-10.
- [ ] Harden the run-id → path resolution: reject path separators or `..` in a run id before joining it to the result store (`arc log <id>`, `arc promote <id>`), so a malformed id can't address files outside `~/.arc/logs/results/`.
