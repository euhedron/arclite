...
*Download & run* (no toolchain — for using arc on your own repos): grab the latest `arc-<version>-<os>` from the repo's **[Downloads](https://github.com/nikganderson/arclite/downloads)** (you'll be signed into GitHub), put it on your `PATH` as `arc` (`arc.exe` on Windows; `chmod +x` on Linux/macOS), then run `arc doctor`. No clone, no Rust. (Right now only the **Windows** binary is published — the per-OS auto-upload is blocked, see the release roadmap item below; on Linux/macOS, use *From source* until that lands.)
