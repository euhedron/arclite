...
- [x] **`arc tui`** — an interactive **cockpit** over the same commands: a launchpad to fire and observe targeted analysis *alongside* the dev agents (not a chat). Inline (preserves the shell's scrollback, not an alt-screen takeover), on a sync input+tick→`mpsc` runtime with render a pure function of state (headless-tested). The spine has landed — a home view, a `/` command palette, a persistent footer carrying the active-run count across every view, and a live `status` view that refreshes in place. Open: launching runs from the palette (`--dry-run`→confirm→live-cost guardrails baked in; v1 = live progress + the verbatim result, with live-text-streaming a follow-up); the remaining browse/edit views (runs/usage/rules/config); and — **the larger aim** — rendering findings and extracted rules as **interactive, structured items acted on in place** (promote a finding into the repo's ledger; pick which rules to merge/dedup), consuming runs' *structured* output rather than rendered text — the natural surface for **Promote findings** and cross-repo **rule aggregation** below.
