...
- **Gating / hooks for any command (cost-visible).** The **gate** ships: `--fail-on-findings` makes any command with a findings collection exit non-zero (code 2) when it's non-empty, now wired into an opt-in, cost-loud git **pre-push** hook. Still open: **Claude Code hook events** invoking `arc` (complementary to git hooks, not a replacement), and whether a pre-commit variant ever earns its keep.
- **Unified findings shape.** Each structured command emits its own collection (`audit` → `violations`, `suggest` → `suggestions`, `extract` → `candidates`); the gate already treats them uniformly (non-empty ⇒ block). Open: whether to collapse the bespoke labels into one **`results`/`candidates`** envelope (a shared key, command-specific item shapes) to simplify schemas and make gating fully label-agnostic — weighed against the clarity a precise label like `violations` carries. (`suggest`-as-gate is a real use-case: a non-empty list flags room for refinement worth blocking on.)
- **Command-kit identity.** The commands are one shared substrate differentiated only by prompt (`suggest` surfaces what's worth attention, `critique` finds defects, `audit` checks rules, `summarize` describes, `extract` mines rules). Watch for overlap/necessity/distinctness as the kit grows; let use — not speculation — justify each verb.
