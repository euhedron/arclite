...
arclite is an **agent-first, cross-platform CLI for cross-repo code intelligence and auditing**. It gathers facts about a repository **deterministically**, and — only where genuine judgment is needed — applies **AI** (via an agent CLI — Claude Code, Codex, or the Cursor CLI, selectable per run). Every use is cost-transparent, configurable, and observable (see [Principles](#principles)). The aim: unlock analysis/auditing that doesn't already exist, while spending *sensibly*.

Open source (MIT), built by [Euhedron](https://euhedron.com) — shipped as **Arc**, from this repo (`euhedron/arclite`). This repo is also arclite's own proving ground: every push is gated by its own audit, and `.arc/findings/` is the live ledger of what those audits caught — the commit history *is* the product demo.

## Getting started

**Prerequisites.** The downloaded binary is self-contained — **no Rust needed to run it** (a Rust toolchain is only for the *from-source* build below: `cargo`, with Rust as new as the crate's `rust-version`, for let-chains). arc's AI commands are powered by an **agent CLI on your `PATH`** — [Claude Code](https://code.claude.com/docs) (`claude`), [Codex](https://developers.openai.com/codex) (`codex`), or [Cursor](https://cursor.com) (`cursor-agent`, on a Cursor subscription); at least one is required, and you select which — per run with `--backend`, or standing via the `backend` setting. `git` grounds synthesis prompts in path-tracking state and backs `--changed`, `arc init --hook`, `arc update`, and per-run commit provenance; `curl` is only for `arc update --apply` and the provider model listings (`arc models`, the TUI's model pickers). The deterministic commands (`inspect`, `doctor`, `status`, …) need none of these. The execution boundary below is audited against Claude Code 2.1.216 and Codex CLI 0.144.4; an older CLI that does not recognize a required flag/config key fails the run instead of silently weakening it. `arc doctor` reports what's installed.

**Install.** Two ways:

*Download & run* (no toolchain — for using arc on your own repos): grab the latest `arc-v<version>-<os>-<arch>` binary for your platform from the repo's **[Releases](https://github.com/euhedron/arclite/releases)**, put it on your `PATH` as `arc` (`arc.exe` on Windows; `chmod +x` on Linux/macOS), then run `arc doctor`. No clone, no Rust. (Binaries are published by the release workflow on each version tag.)
...
- [x] **Run logging + rollup** — mechanism under **Logging** above (each run already stamps arc's release `version`). Open: cross-repo and team aggregation; trends over time (audit pass-rate, cost curves) to see whether the rules earn their keep — concretely a **per-rule firing-rate view** in the CLI/TUI (the data already exists: every finding cites its rule id in the stored result, joinable to the run by id), which needs runs fingerprinted *below* the release `version` — arc's own commit or a rules-content hash — so a rule's firing history is attributable across ruleset edits within a version, not only between releases.
...
- [ ] **Smooth onboarding & rule capture** — `arc init`, then extract → curate → audit, should run end to end without friction. Exercised on foreign repos (a .NET solution; the crux/commissure sibling products), which surfaced and closed real gaps (`--exclude`, `--no-scan`, a top-level `inspect` layout). Now that Arc is public, the bar moves: a *stranger* with a downloaded binary, no sibling context, and no hand-holding is the onboarding that counts — the README + `arc doctor` + `arc init` have to carry it end to end.
...
- **Structured output vs. tool calls.** A command's typed result can come as a final **structured-output** artifact (today's `--json-schema`/`--output-schema` → `results`+`note`) or the model **calling a tool** with structured *input* — near-equivalent in expressiveness, differing in lifecycle (one parse point vs. per-item events + partial salvage), not a settled quality claim. arclite uses structured output; tool use is a lever for when a use-case earns it. (An `audit` once emitted an `extract`-shaped item that slipped the declared schema and gated a push; arclite now **re-validates every structured result locally** against the verb's declared shape before gating/logging/promoting on it — mismatches join the errored path with the offending fields named, elisions disclosed — trusting the channel less.)
