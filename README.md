...
- [ ] **Promote findings into the repo** — collect proven results under the repo's `.arc/findings/` (and/or `~/.arc/`) as a curated, per-scope ledger of accepted, still-open findings. **`arc promote <run-id>` landed (2026-06-23):** the *system* writes a logged run's structured findings into `.arc/findings/open/` (atomic, collision-free names — agents invoke arc, they don't hand-author the ledger). **`--findings` landed (2026-06-24):** a run feeds the open ledger into context (framed *surface new issues beyond these*), so it hunts past the already-known. **`arc run verify` landed (2026-06-28):** the verify lever's *judgment* half — it re-checks the open ledger against the current code and returns a per-finding verdict (`reproduces` | `resolved` | `indeterminate`), each grounded in a cited mechanism (proven first on arclite's own ledger — all three verdicts correct, including a `resolved` it called on its own). Still open: the **retire** half — acting on the verdicts to move `resolved` findings out of the open ledger (via the system, not by hand); auto-loading the ledger so a verify run needs no explicit `--include`; and cross-scope curation (repo vs. `~/.arc`). Floated 2026-06-10.
