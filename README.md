...
*Download & run* (no toolchain — for using arc on your own repos): grab the latest `arc-v<version>-<os>-<arch>` binary for your platform from the repo's **[Downloads](https://github.com/nikganderson/arclite/downloads)** (you'll be signed into GitHub), put it on your `PATH` as `arc` (`arc.exe` on Windows; `chmod +x` on Linux/macOS), then run `arc doctor`. No clone, no Rust. (Right now only the **Windows** binary is published — the per-OS auto-upload is blocked, see the release roadmap item below; on Linux/macOS, use *From source* until that lands.)
...
- **Agent-agnostic?** (Claude Code + Codex + any.) **Distribution** — per-OS binaries on Downloads (via the release pipeline) ship now; `arc update` (check + `--apply` self-replace) has landed; still open: `cargo-binstall`/Homebrew/Scoop, and TUI-integrated auto-update. **CI** — Linux build/test/lint + the tagged release run are in place; macOS needs a runner (Windows is cross-compiled from Linux). **Dashboard / IDE / linter integration?**
