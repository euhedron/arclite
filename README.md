...
- **Agent-agnostic?** (Claude Code + Codex + any.) **Distribution** — per-OS binaries on GitHub Releases (via the release workflow) and `arc update` (check + `--apply` self-replace) ship now; still open: `cargo-binstall`/Homebrew/Scoop, and TUI-integrated auto-update. **CI** — GitHub Actions runs build/test/lint on Linux + macOS, with a tag-triggered release build per platform. **Dashboard / IDE / linter integration?**
