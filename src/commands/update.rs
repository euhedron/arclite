...
fn apply(
    ...
    // the download. Optional — sent to curl only when set (empty = unset), so the public case stays
    // frictionless; a set-but-non-unicode token is an error, not the absent case: silently proceeding
    // unauthenticated would turn a private repo's lookup into a misleading "not found".
    let auth = crate::env_optional(AUTH_ENV, "proceed unauthenticated")?;
